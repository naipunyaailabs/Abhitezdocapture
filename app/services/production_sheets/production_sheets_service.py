...
DATE_SYSTEM_PROMPT = (
    ...
    "factory. At the TOP of the page there are TWO header fields:\n"
    "  - a DATE field, labelled 'DATE :' or 'DD/MM/YYYY :', with a handwritten\n"
    "    date value, sometimes followed by a shift word (DAY / NIGHT); and\n"
    "  - a separate SHIFT field, labelled 'SHIFT :', with the word DAY or NIGHT.\n\n"
    "Your job: TRANSCRIBE the date character-by-character EXACTLY as drawn, and\n"
    "read the SHIFT word.\n\n"
    ...
    "   If the page shows '13/06/026', output exactly \"13/06/026\" — NOT\n"
    "   '13 June', NOT '13 June 2026', NOT '13/06/2026'. Keep the literal digits,\n"
    ...
    "5. If a SHIFT word (DAY or NIGHT) is written INSIDE the date field next to\n"
    "   the digits, do NOT merge it into the date string — put it in 'shift'.\n"
    "6. If no date is visible, output an empty string for date.\n\n"
    "SHIFT RULES:\n"
    "7. The shift is ALWAYS exactly one of two words: DAY or NIGHT. Read the\n"
    "   handwriting and pick whichever is written. Look BOTH next to the date AND\n"
    "   in the dedicated 'SHIFT :' field — they should agree; use either.\n"
    "8. Normalise the shift to UPPERCASE: output exactly \"DAY\" or \"NIGHT\".\n"
    "   If you genuinely cannot tell and nothing is written, output \"\".\n\n"
    "Output JSON ONLY: {\"date\": \"<exact literal date string>\", "
    "\"shift\": \"<DAY|NIGHT|>\"}"
)

DATE_USER_PROMPT = (
    "Read the two header fields at the top of this page.\n"
    "1) DATE: transcribe the value next to 'DATE :' (or 'DD/MM/YYYY :') EXACTLY "
    "as written, character by character — keep all slashes/dots and the literal "
    "digits (e.g. '13/06/026' stays '13/06/026', never '13 June'). Do NOT convert "
    "to a month name or change the year digits.\n"
    "2) SHIFT: read the shift word — it is DAY or NIGHT. Check both next to the "
    "date and in the 'SHIFT :' field. Output it UPPERCASE as 'DAY' or 'NIGHT'.\n"
    "Output JSON: {\"date\": \"<literal>\", \"shift\": \"<DAY|NIGHT|>\"}."
)
